...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const elements = {
        ...
        badgeLanLabel: document.getElementById('badge-lan-label') || document.querySelector('#badge-lan .badge-label'),
    ...
    // --------------------------------------------------------------------------
    // 0. BOOT CONFIG SYNC — fetches ESP_IP from app.py line 48 via /api/config
    //    This runs immediately on page load so every part of the frontend uses
    //    the single-source-of-truth IP before any polling tick fires.
    // --------------------------------------------------------------------------
    async function fetchEspConfig() {
        try {
            const res = await fetch('/api/config');
            if (!res.ok) return;
            const cfg = await res.json();
            if (cfg.esp_ip) {
                appState.espIp = cfg.esp_ip;
                if (elements.badgeLanLabel) {
                    elements.badgeLanLabel.textContent = `LAN HTTP: ${cfg.esp_ip}`;
                }
            }
            if (cfg.lan_target_url) {
                appState.lanTargetUrl = cfg.lan_target_url;
            }
        } catch (err) {
            console.warn('[CONFIG] Could not fetch ESP config from backend:', err.message);
        }
    }
    fetchEspConfig();
    ...
    async function executeGoTo() {
        ...
        try {
            ...
            const espStatus = data.response ? (data.response.status || 'sent') : 'sent';
            const targetUrl = (data.response && data.response.url) ? data.response.url.split('?')[0] : (appState.lanTargetUrl || `http://${appState.espIp || 'ESP_DEVICE'}/target`);
            const targetAltStr = (data.new_alt_deg !== undefined ? data.new_alt_deg : 0).toFixed(3);
            const targetAzStr  = (data.new_az_deg !== undefined ? data.new_az_deg : 0).toFixed(3);
            if (espStatus === 'esp_offline') {
                if (elements.gotoFeedback) elements.gotoFeedback.textContent = `[ESP OFFLINE] Target logged locally (Target: Alt ${targetAltStr}°, Az ${targetAzStr}° | ΔAlt ${dAltStr}°, ΔAz ${dAzStr}°). ESP at ${appState.espIp || 'unknown IP'} is unreachable — connect ESP to receive commands.`;
            } else {
                if (elements.gotoFeedback) elements.gotoFeedback.textContent = `[GOTO DISPATCHED] LAN GET: ${targetUrl}?alt=${dAltStr}&az=${dAzStr} (Target: Alt ${targetAltStr}°, Az ${targetAzStr}°) [${espStatus.toUpperCase()}]`;
            }
            pollHardwareStatus();
        } catch (err) {
    ...
    if (elements.btnCalibStarSubmit) {
        elements.btnCalibStarSubmit.addEventListener('click', async () => {
            if (!appState.selectedTarget) {
                if (elements.calibrationFeedback) {
                    elements.calibrationFeedback.textContent = "[CALIBRATION ERROR] Please select an object from the sky map or search bar first.";
                }
                return;
            }
            ...
            if (elements.calibrationFeedback) {
                elements.calibrationFeedback.textContent = `Aligning mount to active target object "${starName}" (Alt ${formatDegrees(alt)}, Az ${formatDegrees(az)})...`;
            }
            try {
                ...
                const data = await res.json();
                if (elements.calibrationFeedback) elements.calibrationFeedback.textContent = `[CALIBRATION SUCCESS] ${data.description}`;
                pollHardwareStatus();
            } catch (err) {
    ...
    if (elements.btnMapCalibrate) {
        elements.btnMapCalibrate.addEventListener('click', async () => {
            if (!appState.selectedTarget) {
                const noTgtMsg = "[CALIBRATION ERROR] Please select an object on the sky map or search first.";
                if (elements.gotoFeedback) elements.gotoFeedback.textContent = noTgtMsg;
                if (elements.calibrationFeedback) elements.calibrationFeedback.textContent = noTgtMsg;
                return;
            }
            const starName = appState.selectedTarget.name;
            const alt = appState.selectedTarget.altitude;
            ...
            const startMsg = `Aligning mount to map object "${starName}" (Alt ${formatDegrees(alt)}, Az ${formatDegrees(az)})...`;
            if (elements.gotoFeedback) elements.gotoFeedback.textContent = startMsg;
            if (elements.calibrationFeedback) elements.calibrationFeedback.textContent = startMsg;
            ...
            try {
                ...
                const data = await res.json();
                const successMsg = `[CALIBRATION SUCCESS] ${data.description}`;
                if (elements.gotoFeedback) elements.gotoFeedback.textContent = successMsg;
                if (elements.calibrationFeedback) elements.calibrationFeedback.textContent = successMsg;
                pollHardwareStatus();
            } catch (err) {
                const errMsg = `Calibration error: ${err.message}`;
                if (elements.gotoFeedback) elements.gotoFeedback.textContent = errMsg;
                if (elements.calibrationFeedback) elements.calibrationFeedback.textContent = errMsg;
            }
        });
    }
    ...
    async function pollHardwareStatus() {
        ...
        try {
            ...
            if (hwData.esp_ip) appState.espIp = hwData.esp_ip;
            if (hwData.lan_target_url) appState.lanTargetUrl = hwData.lan_target_url;
            if (elements.badgeLanLabel && hwData.esp_ip) {
                elements.badgeLanLabel.textContent = `LAN HTTP: ${hwData.esp_ip}`;
            }
